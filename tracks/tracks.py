...
def zl(stdscr):
    stdscr.clear()
    n = tracks.station.StationBrowser()
    n.browse(stdscr)
...
def main():
    ...
    if (args.addlink is not None and args.filename is not None):
        ...
        note1 = tracks.station.Station(args.filename)
        note2 = tracks.station.Station(args.addlink)
        ...
        #  and no holes
        files = tracks.station.loadnotes()
        for i in range(0, len(files)):
            n = None
            try:
                n = tracks.station.Station(files[i])
            except:
            ...
            if (n.order != i+1):
                ...
                for file in files:
                    scannote = tracks.station.Station(file)
