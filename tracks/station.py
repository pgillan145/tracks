...
import tracks
import tracks.globalvars

from tracks.file import FileBrowser, File
    ...
    def view(self, stdscr):
        ...
        while (True):
            ...
            if (command == "KEY_DC" or command == ""):
                ...
                if (tracks.globalvars.link_note is None):
                    # store this note for linking later
                    tracks.globalvars.link_note = self
                else:
                    # link the previous note to the current note
                    self.addnotelink(tracks.globalvars.link_note)
                    self.write()
                    tracks.globalvars.link_note.addnotebacklink(self)
                    tracks.globalvars.link_note.write()
                    tracks.globalvars.link_note = None
            elif (command == "p"):
                if (tracks.globalvars.link_filename and tracks.globalvars.link_text):
                    link = Link(tracks.globalvars.link_filename)
                    ref = Reference(link, tracks.globalvars.link_text)
                    self.addreference(ref)
                    self.write()
                elif(tracks.globalvars.copy):
                    self.addreference(tracks.globalvars.copy)
                    self.write()
            elif (command == "q"):
                sys.exit()
            elif (command == "r"):
                ...
                if (select is True):
                    if (mark_x is not None):
                        ...
                        tracks.globalvars.copy = Reference(link, text)
...
class NoteBrowser():
    def browse(self, stdscr, filename=None):
        ...
        while (command != "q"):
            ...
            if (note1 is not None):
                ...
                if (tracks.globalvars.reload):
                    ...
                    try:
                        ...
                        pass
                    tracks.globalvars.reload = False
            ...
            if (tracks.globalvars.link_note is not None):
                status = f"{status} LINKING"
            if (tracks.globalvars.filter != ""):
                status = f"{status} FILTERED:'{tracks.globalvars.filter}'"
            ...
            #    note = Note(files[selected])
                ...
                if (tracks.globalvars.filter != ""):
                    continue
                if (move is True or tracks.globalvars.link_note is not None):
                    move = False
                    tracks.globalvars.link_note = None
                    continue
                note = Note(files[selected])
                ...
                if (new_filter != ""):
                    tracks.globalvars.filter = new_filter
                if (tracks.globalvars.filter == ""):
                    continue
                ...
                if (tracks.globalvars.link_note is None):
                    # store this note for linking later
                    tracks.globalvars.link_note = note
                elif (note is not None):
                    # link the previous note to the current note
                    note.addnotelink(tracks.globalvars.link_note)
                    note.write()
                    tracks.globalvars.link_note.addnotebacklink(note)
                    tracks.globalvars.link_note.write()
                    tracks.globalvars.link_note = None
            elif (command == 'm'):
                ...
                # clear any 'special' modes.
                if (move == True or tracks.globalvars.link_note != None):
                    move = False
                    tracks.globalvars.link_note = None
                elif (tracks.globalvars.filter != ""):
                    tracks.globalvars.filter = ""
                    f = files[selected]
                    files = loadnotes()
                    try:
...
def loadnotes():
    ...
    for f in os.listdir("."):
        if (os.path.isfile(os.path.join(".", f)) and re.search("^\d+ - .+\.md$",f)):
            if (tracks.globalvars.filter != ""):
                note = Note(f)
                logging.debug("filtering for %s", tracks.globalvars.filter)
                if (note.search(tracks.globalvars.filter) == False):
                    continue
                logging.debug("  found")
            files.append(f)
